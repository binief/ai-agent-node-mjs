...
function parseTextToolCalls(text) {
  const t = String(text || "");
  const calls = [];
  if (!/\x3cfunction=/i.test(t)) return calls;
  ...
  while ((m = fnRe.exec(t))) {
    ...
    let pm;
    while ((pm = pRe.exec(body))) args[pm[1]] = pm[2].trim();
    if (!Object.keys(args).length) {
      const flat = body.replace(/\x3c\/?parameter\x3e?/gi, "").trim();
      if (flat) args.command = flat;
    }
    calls.push({ id: "", name, arguments: JSON.stringify(args) });
  }
  return calls;
}
function stripToolMarkup(t) {
  return String(t)
    ...
    .replace(/\x3c\/?tool_call\x3e/gi, "")
    .replace(/\x3c\/?function=[^\x3e]*\x3e/gi, "")
    .replace(/\x3c\/function\x3e/gi, "")
    .replace(/\x3c\/?parameter=[^\x3e]*\x3e/gi, "")
    .replace(/\x3c\/parameter\x3e/gi, "")
    .trim();
}
  ...
  switch (cmd) {
    ...
    case "/set": {
      try {
        ...
        else { console.log(dim("usage: /set url|model|key|draft_model|temperature|reasoning|redact|dir|context|max_tokens|maxout|intercept|max_steps|system <value>")); return true; }
        saveCfg(cfg);
...
function banner(cfg) {
  ...
  console.log(bold("◆ ai-agent") + dim(`  ${cfg.model} @ ${cfg.apiUrl}  ·  ${osDescription()}  ·  ctx ${cfg.context ? fmtK(cfg.context) : "unknown"}  ·  key ${cfg.apiKey ? "✓" : "—"}` + (cfg.draftModel ? `  ·  draft ${cfg.draftModel}` : "") + (cfg.temperature != null ? `  ·  temp ${cfg.temperature}` : "") + (cfg.reasoning ? `  ·  reasoning ${cfg.reasoning}` : "") + (cfg.intercept ? "  ·  🔒 intercept" : "")));
